...
function exportToCSV() {
    let csvContent = "data:text/csv;charset=utf-8,Date,Type,Montant (€)\n"; // Adding the "Date" column
    
    tickets.forEach(ticket => {
        csvContent += `${ticket.date},${ticket.type},${ticket.amount}\n`;
    });

    let encodedUri = encodeURI(csvContent);
    let link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", `tickets_${new Date().toISOString().split("T")[0]}.csv`); // Ajoute la date du jour au nom du fichier
